# Development Notes - ObsidianQuickLaunch

This document captures key technical decisions, challenges solved, and the rationale behind implementation choices during development.

## Table of Contents
- [Session: Phase 2 Implementation & Bug Fixes (Feb 13-14, 2026)](#session-phase-2-implementation--bug-fixes-feb-13-14-2026)
- [Session: Phase 2 Design - Templates & File Association (Feb 13, 2026)](#session-phase-2-design---templates--file-association-feb-13-2026)
- [Session: WiX Installer Build System (Dec 30, 2024)](#session-wix-installer-build-system-dec-30-2024)
- [Session: Initial Development (Dec 12-16, 2024)](#session-initial-development-dec-12-16-2024)

---

## Session: Phase 2 Implementation & Bug Fixes (Feb 13-14, 2026)

### Goal
Implement the full Phase 2 feature set (templates, chooser, .md association) and fix bugs found during testing.

### Implementation Summary

**Scripts Created:**
- `src/scripts/choose-template.ps1` — Windows Forms template chooser dialog
- `src/scripts/open-md-file.ps1` — .md file handler (opens parent folder as vault)

**Scripts Modified:**
- `src/scripts/register-vault-final.ps1` — Added `-TemplateName` parameter, `Copy-TemplateNonDestructive` function, template resolution logic (user dir → bundled fallback)
- `src/scripts/install-context-menu.ps1` — Added template chooser registry entries, updated text to "QuickLaunch Obsidian here"
- `src/scripts/uninstall-context-menu.ps1` — Added cleanup of template chooser registry keys
- `tools/build/Build-Installer.ps1` — Added new scripts and templates to staging
- `tools/packaging/wix/Product.wxs` — Major changes (see below)

**Templates Created:**
- `src/templates/default/.obsidian/app.json` — Minimal default
- `src/templates/zettelkasten/` — Daily Notes + Templates folders
- `src/templates/project-docs/` — docs folder + starter README

**Tests Created:**
- `tests/test-template-application.ps1` — 13 tests (all passing)
- `tests/test-template-chooser.ps1` — 9 tests (all passing)
- `tests/test-md-file-open.ps1` — 7 tests (all passing)

### Key Design Changes from Original Plan

#### Extended Registry Entries Removed (Bug #5)
**Original plan:** Use Windows `Extended` registry value so template chooser only appears with Shift+right-click.

**Problem:** Windows 11's modern context menu does not support the `Extended` mechanism. The modern menu intercepts right-click events and only shows a limited set of verbs. `Extended` entries only work in the classic context menu (accessible via "Show more options"), and even then Shift+right-click doesn't reliably trigger them through the modern menu layer.

**Resolution:** Template chooser entries are now **always visible** in the context menu. Both entries have distinct labels:
- "QuickLaunch Obsidian here" — applies default template silently
- "QuickLaunch Obsidian here (choose template)..." — shows template chooser dialog

This is a better UX anyway — the feature is discoverable without requiring users to know about a hidden Shift+click interaction.

#### Context Menu Icon via VBScript Custom Action (Bug #3)
**Problem:** The MSI installer wasn't setting the Obsidian icon on context menu entries. The manual install script (`install-context-menu.ps1`) handled this correctly via PowerShell regex parsing, but `Product.wxs` didn't include Icon registry values.

**Challenge:** The `OBSIDIANPATH` property from `RegistrySearch` returns the raw command string (e.g., `"D:\bin\Obsidian\Obsidian.exe" "%1"`). Windows Icon registry values need just the exe path with an icon index.

**Solution:** Added a VBScript `CustomAction` that runs after `AppSearch` to parse the quoted exe path from `OBSIDIANPATH` and set a clean `OBSIDIANICON` property (e.g., `D:\bin\Obsidian\Obsidian.exe,0`). Registry entries reference `[OBSIDIANICON]`.

#### .md File Association Level Behavior (Bug #1)
**Issue:** User reported "Reset key enables .md file association" — expected it to be opt-in only.

**Analysis:** With `WixUI_Mondo` and `Level="2"`:
- **Typical install** (INSTALLLEVEL=1): Feature NOT installed ✓
- **Complete install** (INSTALLLEVEL=32767): Feature IS installed — this is standard "Complete" behavior
- **Reset in Custom dialog**: Resets to state based on INSTALLLEVEL from the install type selection

**Resolution:** This is standard WiX behavior. "Complete" means "install everything." Added `Absent="allow"` and updated the feature description to clarify. Users who want Typical without .md association should use Typical install, not Complete.

### WiX Product.wxs Changes Summary

1. **UI:** `WixUI_InstallDir` → `WixUI_Mondo` (for feature selection + directory browsing)
2. **Feature:** Added `ConfigurableDirectory="INSTALLFOLDER"` on MainFeature (enables Browse in Custom)
3. **Feature:** Added `MdFileAssociation` (Level=2, opt-in, AllowAdvertise=no, Absent=allow)
4. **Custom Action:** VBScript `ParseObsidianIcon` to extract Obsidian exe path
5. **Registry:** Added `Icon` values referencing `[OBSIDIANICON]` on all 4 context menu entries
6. **Registry:** Changed context menu text to "QuickLaunch Obsidian here"
7. **Registry:** Removed `Extended` values from template chooser entries
8. **Registry:** Added `.md` file association components (`OpenWithProgIds`, `ProgId` shell entries)

### Testing Status

All tests pass. **User testing needed** for:
1. Install fresh MSI, verify all 4 context menu entries appear with Obsidian icon
2. Right-click a folder → "QuickLaunch Obsidian here" → verify vault opens with default template
3. Right-click a folder → "QuickLaunch Obsidian here (choose template)..." → verify dialog appears
4. Typical install → verify .md association is NOT installed
5. Custom install → enable .md → verify double-click .md opens vault
6. Verify app.json only appears in `.obsidian/` folder, NOT vault root
7. Uninstall → verify all registry entries cleaned up

### Session Summary

**Completed:** Full Phase 2 implementation + 5 bug fixes.

**Key Deliverables:**
- Template system with non-destructive file copy
- Template chooser dialog (Windows Forms)
- .md file association (opt-in via MSI)
- 3 bundled templates (default, zettelkasten, project-docs)
- 29 automated tests across 3 test files
- VBScript custom action for icon detection
- All context menu entries renamed and icon-enabled

**Status:** ✅ Phase 2 complete. MSI builds successfully. Needs user testing.

---

## Session: Phase 2 Design - Templates & File Association (Feb 13, 2026)

### Goal
Design the template system for vault creation and add .md file association support.

### Decision: Templates as Plain Folders

**Approach:** Templates are simply complete vault directory structures (`.obsidian/` folder plus any starter files/folders). No custom config format, no metadata files — just a folder that gets copied.

**Rationale:**
- Zero learning curve — users already know how to set up an Obsidian vault
- Creating a template = setting up a vault the way you want it, then copying the folder
- No maintenance burden from a custom schema
- Easy to share templates (just zip a folder)

### Template Storage

**Location:** `%APPDATA%\ObsidianQuickLaunch\templates\`

Each subfolder is a template, named by its folder name:
```
%APPDATA%\ObsidianQuickLaunch\templates\
├── default\              # Always applied when no selection made
│   └── .obsidian\
│       └── ...
├── Zettelkasten\
│   ├── .obsidian\
│   ├── Daily Notes\
│   └── Templates\
└── Project Docs\
    ├── .obsidian\
    ├── docs\
    └── README.md
```

**Bundled templates** ship with the MSI installer as examples. Users can add, modify, or remove templates freely.

### Template Application Rules

1. **Applied at vault creation only** — never retroactively to existing vaults
2. **Non-destructive copy** — file-level check; never overwrite existing files. A template can add files into an existing `.obsidian/` folder without clobbering existing settings
3. **Default template** is always applied silently on every "Open as Obsidian Vault" action
4. **Vault opening order** is unchanged — previously open vaults first, new vault last

### Template Selection UX

**Normal right-click:** "QuickLaunch Obsidian here" — applies the default template silently.

**Second context menu entry:** "QuickLaunch Obsidian here (choose template)..." — shows a template chooser dialog (only if more than one template exists; otherwise behaves like normal).

**Implementation:** ~~Originally planned to use Windows `Extended` registry entry for Shift+right-click, but this is incompatible with Windows 11's modern context menu.~~ Both entries are always visible with distinct labels.

```
HKCR\Directory\shell\ObsidianQuickLaunch
├── (Default) = "QuickLaunch Obsidian here"
├── Icon = [Obsidian.exe path],0
└── command\...

HKCR\Directory\shell\ObsidianQuickLaunchTemplate
├── (Default) = "QuickLaunch Obsidian here (choose template)..."
├── Icon = [Obsidian.exe path],0
└── command\...
```

Same pattern for `Directory\Background\shell\`.

**Template chooser dialog:** Simple Windows Forms dialog with a list of available template names. Minimal — just a listbox and OK/Cancel.

### .md File Association (New Feature)

**Behavior:**
1. User double-clicks any `.md` file
2. Handler script determines the file's immediate parent folder
3. Registers/opens that folder as an Obsidian vault (same logic as context menu)
4. Obsidian opens with the file's vault visible

**Design decisions:**
- **Immediate parent folder** is used as the vault root (simple, predictable)
- **Opt-in during install** — unchecked checkbox by default, since this is a system-visible change that could conflict with VS Code, Typora, or other .md editors
- Reuses existing vault registration logic from `register-vault-final.ps1`

**Registry:**
```
HKCR\.md\OpenWithProgIds\ObsidianQuickLaunch.md
HKCR\ObsidianQuickLaunch.md\shell\open\command = powershell.exe ... open-md-file.ps1 "%1"
```

### Key Technical Considerations

1. **Extended context menu entries** are natively supported by Windows — add an `Extended` string value (empty) to the registry key and Windows only shows the entry when Shift is held
2. **Non-destructive file copy** must be at the file level, not folder level, so templates can add files into existing directories (e.g., adding plugins to an existing `.obsidian/` folder)
3. **File association** requires a new WiX feature component (opt-in checkbox) and a new script (`open-md-file.ps1`) that wraps the vault registration logic

### Session Summary

**Completed:** Phase 2 design decisions finalized. No code written — design only. (Implementation in next session.)

**Key Deliverables:**
- Template system design (plain folders, non-destructive copy, default + chooser)
- Template selection UX (normal click = default, Shift+click = chooser dialog)
- .md file association design (opt-in, immediate parent folder as vault)
- Updated TODO.md with refined task breakdown

---

## Session: WiX Installer Build System (Dec 30, 2024)

### Goal
Create a professional MSI installer for ObsidianQuickLaunch following Microsoft PowerShell project's build patterns.

### Decision: WiX Toolset 3.x with PowerShell Build Scripts

**Rationale:**
- Follows Microsoft PowerShell's proven installer build pattern
- Professional MSI installer with proper Windows integration
- Automatic registry management with rollback support
- Uninstallation through Add/Remove Programs
- Version upgrade management built-in

**Reference Implementation:**
Studied [PowerShell/PowerShell](https://github.com/PowerShell/PowerShell/tree/master/tools/packaging) build system to understand:
- How they use PowerShell scripts to generate WiX files dynamically
- Directory structure: `tools/packaging/` for build infrastructure
- Packaging module pattern with reusable functions
- WiX file harvesting with `heat.exe`
- Build orchestration approach

### Architecture: Three-Layer Build System

1. **Build Script** (`Build-Installer.ps1`)
   - User-facing entry point
   - Auto-detects version from release-please manifest
   - Creates staging directory with files to package
   - Provides colored output and error handling
   - Clean build support

2. **Packaging Module** (`tools/packaging/packaging.psm1`)
   - PowerShell module following standard patterns
   - `New-MSIPackage`: Main build function
   - `Get-WixPath`: Locates WiX Toolset on system
   - `Start-MsiBuild`: Compiles WiX source to MSI
   - `New-MsiArgsArray`: Converts hashtables to WiX preprocessor args

3. **WiX Source** (`tools/packaging/wix/Product.wxs`)
   - Declarative XML defining installer structure
   - Registry integration for context menus
   - Component organization following WiX best practices
   - **UpgradeCode**: `A7F8B9C0-1D2E-4F5A-8B7C-9D6E5F4A3B2C` (permanent!)

### Key Technical Decisions

#### Dynamic File Harvesting
Uses WiX `heat.exe` to auto-generate file manifests rather than hardcoding in Product.wxs.

**Benefits:**
- No manual file list maintenance
- Automatically includes all staging files
- Generates stable Component GUIDs

**Process:**
```powershell
heat.exe dir $staging -dr INSTALLFOLDER -cg ProductFiles -ag -sfrag -srd -out Fragment.wxs
```

#### Registry-Based Obsidian Icon Detection
Installer attempts to find Obsidian's icon from registry for context menu:

```xml
<RegistrySearch Id="SearchObsidianPath" Root="HKCR"
                Key="obsidian\shell\open\command" Type="raw" />
```

**Graceful Degradation:** Context menu works without icon if Obsidian not found.

#### Staging Directory Pattern
Build creates `tools/packaging/staging/` before running WiX.

**Advantages:**
- Clean separation source vs packaged files
- Allows future file transformations
- Easy cleanup with `-Clean`
- Matches PowerShell project structure

#### Context Menu Registry Structure

**Directory Context (right-click ON folder):**
```
HKCR\Directory\shell\ObsidianQuickLaunch
├── (Default) = "QuickLaunch Obsidian here"
├── Icon = [Obsidian.exe path],0
└── command\(Default) = powershell.exe ... register-vault-final.ps1 "%V"
```

**Background Context (right-click IN folder):**
```
HKCR\Directory\Background\shell\ObsidianQuickLaunch
├── (Default) = "QuickLaunch Obsidian here"
├── Icon = [Obsidian.exe path],0
└── command\(Default) = powershell.exe ... register-vault-final.ps1 "%V"
```

**Why HKCR?**
- Affects all users (system-wide context menus)
- Requires admin elevation
- Proper place for shell extensions per Windows guidelines

#### Version Format Conversion
WiX requires 4-part version (`Major.Minor.Build.Revision`):
```powershell
# Input: "1.0.0" from release-please
# Output: "1.0.0.0" for MSI
$msiVersion = "$($versionParts[0]).$($versionParts[1]).$($versionParts[2]).0"
```

### Component Structure

**Features:**
1. **ProductFiles** (auto-generated by heat.exe)
   - PowerShell scripts from `src/scripts/`
   - Documentation files
   - Dynamic file list

2. **ContextMenuRegistration** (manually defined)
   - Directory\shell registry keys
   - Directory\Background\shell registry keys
   - Installation tracking in HKLM

3. **StartMenuShortcuts**
   - Uninstall shortcut
   - Manual install/uninstall script shortcuts (for advanced users)

### Build Artifacts (All Gitignored)

```
dist/
├── ObsidianQuickLaunch-{version}.msi    # Final installer
└── ObsidianQuickLaunch-{version}.wixpdb # Debug symbols

tools/packaging/staging/                  # Temp build files
%TEMP%\Fragment.wxs                       # Heat-generated manifest
%TEMP%\*.wixobj                           # Compiled objects
```

### File Encoding Critical Path

**UTF-8 Without BOM Required:**
- All PowerShell scripts (`*.ps1`)
- Obsidian JSON files
- Reason: Obsidian's JSON parser fails with BOM

**UTF-8 (BOM OK):**
- WiX source files (`*.wxs`)

### Future Enhancements

**Code Signing:**
```powershell
signtool sign /f cert.pfx /p password /t http://timestamp.digicert.com dist/*.msi
```

**GitHub Actions:**
```yaml
- name: Install WiX
  run: choco install wixtoolset -y
- name: Build Installer
  run: .\Build-Installer.ps1
```

**Chocolatey Package:**
Create `ObsidianQuickLaunch.nuspec` for community repository.

### Testing Strategy

**Manual:**
1. Build: `.\Build-Installer.ps1 -Clean`
2. Install with logging: `msiexec /i dist\*.msi /l*v install.log`
3. Verify: Context menu, files, registry, start menu
4. Test vault opening functionality
5. Uninstall via Add/Remove Programs
6. Verify complete cleanup

**Future Automated:**
- Pester tests for build functions
- WiX validation with `smoke.exe`
- VM snapshot testing
- Registry comparison pre/post install

### Lessons Learned

1. **UpgradeCode is Permanent**: Once released, it must never change
2. **Heat.exe Patterns**: File harvesting needs consistent directory structure
3. **Component KeyPaths**: Critical for proper install/uninstall behavior
4. **Registry Permissions**: HKCR requires elevation, can't do per-user
5. **WiX 3 vs 4**: Stick with v3 for established patterns and tooling
6. **64-bit Components**: WiX components must match directory architecture (Win64="yes" for 64-bit Program Files)
7. **XSLT Transforms**: heat.exe doesn't support -platform flag natively; use XSLT transforms to add Win64 attribute
8. **Build Interface**: Action-based commands (build/rebuild/clean) are more intuitive than switches for developers

### References
- [PowerShell Project Build System](https://github.com/PowerShell/PowerShell/tree/master/tools/packaging)
- [WiX v3 Documentation](https://wixtoolset.org/documentation/manual/v3/)
- [Windows Installer Best Practices](https://docs.microsoft.com/windows/win32/msi/)
- [Shell Context Menu Handlers](https://docs.microsoft.com/windows/win32/shell/context-menu-handlers)

### Session Summary

**Completed:** Professional WiX Toolset 3.x MSI installer build system following Microsoft PowerShell project patterns.

**Key Deliverables:**
- `Build-Installer.ps1` - Action-based build script (build/rebuild/clean)
- `tools/packaging/packaging.psm1` - PowerShell module with WiX build functions
- `tools/packaging/wix/Product.wxs` - WiX installer definition
- `tools/packaging/wix/Win64.xslt` - XSLT transform for 64-bit components
- `notes/BUILD.md` - Comprehensive build documentation

**Challenges Solved:**
1. ICE80 error (32-bit components in 64-bit directories) - Fixed with Win64="yes" attributes and XSLT transform
2. heat.exe 64-bit support - Created XSLT transform to add Win64 attribute to generated components
3. Build interface design - Switched from PowerShell switches to action-based commands for better UX

**Status:** ✅ Build system complete and functional. MSI installer successfully creates and registers context menu entries.

**Future Work:**
- ~~Change context menu text to "QuickLaunch Obsidian here"~~ ✅ Done (Phase 2)
- ~~Add custom icon for installer/application~~ ✅ Done (Phase 2 - VBScript custom action)
- Set up GitHub Actions for automated builds

---

## Session History: Initial Development (Dec 12-16, 2024)

### Project Genesis

**Goal**: Create a Windows Explorer context menu integration to quickly open folders as Obsidian vaults.

**User Requirements**:
- Power user workflow with multiple small vaults
- Right-click context menu (both ON folders and IN folders)
- Preserve existing open vaults when opening new ones
- Minimal disruption to existing workflow

---

## Technical Challenges & Solutions

### Challenge 1: How to Open Obsidian Vaults

**Investigation**: Tested multiple approaches to determine the best method.

**Approaches Tested**:
1. ✅ **`obsidian://` URI Protocol** - WORKS
   - Uses `obsidian://open?path=<encoded_path>`
   - Officially supported by Obsidian
   - Requires vault to be registered in `obsidian.json`

2. ❌ **Direct Executable with Arguments** - DOESN'T WORK
   - Tried: `Obsidian.exe "C:\path\to\vault"`
   - Result: Opens Obsidian but ignores the path argument
   - Conclusion: Obsidian doesn't accept command-line vault paths

3. ❌ **Alternative URI Formats** - DOESN'T WORK
   - Tried: `obsidian://vault/<path>`
   - Tried: `obsidian://open?vault=<path>`
   - Result: "Vault not found" errors for unregistered vaults

**Decision**: Use `obsidian://` URI protocol with vault registration in `obsidian.json`.

**Files**: `tests/test-vault-opening.ps1`, `tests/test-direct-launch.ps1`

---

### Challenge 2: Vault Registration Without Restart

**Problem**: Obsidian doesn't hot-reload its vault list.

**Investigation**:
- Modified `obsidian.json` while Obsidian was running
- Attempted to open the newly added vault
- Result: "Vault not found" error

**Critical Discovery**: Obsidian writes to `obsidian.json` on exit, **overwriting** any changes made while it was running.

**Implications**:
- MUST close Obsidian before modifying `obsidian.json`
- Cannot add vaults without interrupting the user's workflow

**Decision**:
1. Save which vaults are currently open
2. Close Obsidian
3. Modify `obsidian.json` safely
4. Reopen all previously open vaults + new vault

**Files**: `tests/test-hot-reload.ps1`, `src/scripts/register-vault-final.ps1`

---

### Challenge 3: Preserving User Workspace

**Problem**: User has multiple vaults open simultaneously. How to avoid losing their workspace?

**Solution**:
1. Before closing Obsidian, parse `obsidian.json` to find vaults with `"open": true`
2. Store list of open vault paths
3. After registration, reopen all previously open vaults
4. Open new vault LAST (so it appears on top/in focus)

**Workspace Restoration**:
- Each vault's state is stored in `.obsidian/workspace.json`
- Obsidian automatically restores workspace when vault opens
- Includes: open notes, tabs, active file, sidebar state, pane layout

**Additional Consideration**: When Obsidian isn't running, skip the close/reopen cycle entirely.

**Files**: `src/scripts/register-vault-final.ps1:36-69`

---

### Challenge 4: Homepage Plugin Conflict

**Issue**: User reported that vaults were opening to homepage instead of previous tabs.

**Investigation**:
- Examined user's vault `D:\1\GitRepos\ScottKirvan\Vaults\sk\.obsidian\plugins/homepage/data.json`
- Found: `"openOnStartup": true` and `"openMode": "Replace all open notes"`

**Root Cause**: The homepage plugin was overriding workspace restoration by replacing all tabs on startup.

**Solution**: Documented the conflict with two options:
1. Set `"openOnStartup": false` (recommended)
2. Set `"openMode": "Keep open notes"` (keeps homepage but restores tabs too)

**Decision**: Not a bug in our tool, but a plugin configuration issue. Documented in README and script comments.

**Files**: `src/scripts/register-vault-final.ps1:3-17`, `notes/README.md:75-94`

---

### Challenge 5: Finding Obsidian Installation

**Problem**: Obsidian can be installed in various locations. How to find the executable for the context menu icon?

**Initial Approach**: Hardcoded common paths:
```powershell
$obsidianIconPaths = @(
    "D:\bin\Obsidian\Obsidian.exe",
    "$env:LOCALAPPDATA\Obsidian\Obsidian.exe",
    "$env:LOCALAPPDATA\Programs\Obsidian\Obsidian.exe"
)
```

**User Request**: "Can we find the obsidian path from the registry rather than hard-coding it?"

**Better Solution**: Query the registry where the `obsidian://` protocol is registered:
```powershell
$regKey = Get-ItemProperty -Path "Registry::HKEY_CLASSES_ROOT\obsidian\shell\open\command"
$command = $regKey.'(default)'  # e.g., "D:\bin\Obsidian\Obsidian.exe" "%1"
```

**Benefits**:
- Works with any installation location
- Uses the same path that Obsidian itself registered
- Automatic fallback to common paths if registry lookup fails

**Files**: `src/scripts/install-context-menu.ps1:51-87`

---

### Challenge 6: Multi-Window Behavior

**Investigation**: Does Obsidian support multiple vault windows simultaneously?

**Test**: Attempted to open multiple vaults via URI while one was already open.

**Result**:
- Obsidian DOES support multiple windows (one per vault)
- Each vault window is independent
- Each restores its own workspace state

**Implication**: Our approach of reopening all previously open vaults works correctly - each opens in its own window.

**Files**: `tests/test-multi-window.ps1`

---

## Design Decisions

### Why PowerShell Scripts Instead of Compiled Executable?

**Pros**:
- Easy to read and modify
- No compilation step
- Users can inspect the code (open source transparency)
- Fast iteration during development
- Native Windows integration

**Cons**:
- Requires PowerShell (built into Windows 10/11)
- Execution policy considerations
- Less "polished" than compiled app

**Decision**: Start with PowerShell, consider compiled packaging later (Phase 2).

---

### Why Registry Modification Instead of Obsidian Plugin?

**Registry Approach**:
- ✅ Works outside of Obsidian
- ✅ System-wide context menu
- ✅ No dependency on Obsidian plugin API
- ❌ Requires administrator privileges

**Plugin Approach**:
- ✅ No admin privileges needed
- ❌ Only works from within Obsidian
- ❌ Doesn't provide context menu in Explorer

**Decision**: Registry integration for Windows Explorer context menu. Plugin approach wouldn't meet the requirement of "right-click any folder in Explorer."

---

### Context Menu: Two Entry Points

**Requirement**: User wanted to open vaults in two scenarios:
1. Right-click ON a folder (from parent directory)
2. Right-click IN a folder (inside the folder to be vaulted)

**Implementation**:
- `HKEY_CLASSES_ROOT\Directory\shell\ObsidianQuickLaunch` - Right-click ON folder
- `HKEY_CLASSES_ROOT\Directory\Background\shell\ObsidianQuickLaunch` - Right-click IN folder

Both use `%V` to get the folder path and call the same script.

**Files**: `src/scripts/install-context-menu.ps1:31-80`

---

## JSON Manipulation Gotchas

### UTF-8 BOM Issues

**Problem**: When using PowerShell's `ConvertTo-Json | Set-Content`, it was adding a UTF-8 BOM that corrupted the file.

**Solution**: Use explicit UTF-8 encoding without BOM:
```powershell
[System.IO.File]::WriteAllText($path, $json, [System.Text.UTF8Encoding]::new($false))
```

**Files**: `src/scripts/register-vault-final.ps1:60`

---

### String Escaping in JSON

**Problem**: Windows paths need double backslashes in JSON:
```
D:\path\to\vault  →  D:\\path\\to\\vault
```

**Solution**:
```powershell
$normalizedPath = $VaultPath -replace '\\', '\\\\'
```

**Files**: `src/scripts/register-vault-final.ps1:71`

---

### Backup Strategy

**Decision**: Always backup `obsidian.json` before modification with timestamp:
```powershell
$timestamp = Get-Date -Format "yyyyMMdd-HHmmss"
$backupPath = "$obsidianConfigPath.$timestamp.bak"
Copy-Item $obsidianConfigPath $backupPath -Force
```

**Rationale**: User's vault list is critical data. If modification fails, they can restore from backup.

**Files**: `src/scripts/register-vault-final.ps1:76-78`

---

## PowerShell Best Practices Applied

### 1. Error Handling
- Try/catch blocks with silent fallbacks
- Graceful degradation (e.g., icon fallback if Obsidian not found)
- User-friendly error messages

### 2. User Feedback
- Color-coded output (Green=success, Yellow=warning, Red=error, Gray=info)
- Progress indicators for multi-step operations
- Clear instructions in output

### 3. Admin Privilege Checks
```powershell
$isAdmin = ([Security.Principal.WindowsPrincipal] `
    [Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole(
    [Security.Principal.WindowsBuiltInRole]::Administrator)
```

### 4. Path Handling
- Always resolve relative paths to absolute
- Quote paths with spaces
- Use `Join-Path` for cross-platform compatibility

---

## Testing Methodology

### Test Scripts Created

All test scripts in `/tests/` directory:

1. **test-vault-opening.ps1** - Tests URI protocol methods
2. **test-multi-window.ps1** - Tests multiple vault windows
3. **test-hot-reload.ps1** - Tests config modification while running
4. **test-direct-launch.ps1** - Tests direct executable launch

**Purpose**: These scripts allowed rapid experimentation to determine what worked vs. what didn't, without writing production code.

---

## Future Considerations

### Phase 2: Template System

**Design finalized** — see [Session: Phase 2 Design (Feb 13, 2026)](#session-phase-2-design---templates--file-association-feb-13-2026) for full details.

### Testing & CI/CD

**Planned**:
- PowerShell Pester unit tests
- GitHub Actions for automated testing
- Test matrix: Different Windows versions, Obsidian versions, install locations

### Distribution

**Options**:
- MSI installer (Windows Installer)
- Chocolatey package
- WinGet package
- Scoop manifest
- Self-extracting executable

---

## Lessons Learned

1. **Always test assumptions**: The assumption that `Obsidian.exe <path>` would work was wrong. Testing revealed the truth.

2. **Registry is powerful**: Windows registry provides a wealth of information about installed applications.

3. **User workflow matters**: The requirement to preserve open vaults fundamentally shaped the architecture.

4. **Documentation is critical**: Without the `.clinerules` file, future sessions would lose all this context.

5. **PowerShell gotchas**: BOM encoding, string escaping, and path quoting all caused issues during development.

---

## Key File Reference

### Production Scripts
- `src/scripts/register-vault-final.ps1` - Main vault registration logic + template application
- `src/scripts/install-context-menu.ps1` - Registry installer (4 context menu entries)
- `src/scripts/uninstall-context-menu.ps1` - Registry cleanup (4 entries)
- `src/scripts/choose-template.ps1` - Template chooser dialog (Windows Forms)
- `src/scripts/open-md-file.ps1` - .md file handler (opens parent folder as vault)

### Templates
- `src/templates/default/` - Minimal default template (`.obsidian/app.json`)
- `src/templates/zettelkasten/` - Zettelkasten template (Daily Notes, Templates)
- `src/templates/project-docs/` - Project documentation template (docs, README)

### Test Scripts
- `tests/test-vault-opening.ps1` - URI protocol validation
- `tests/test-multi-window.ps1` - Multi-window behavior
- `tests/test-hot-reload.ps1` - Hot-reload investigation
- `tests/test-direct-launch.ps1` - Direct exe launch test
- `tests/test-template-application.ps1` - Template copy logic (13 tests)
- `tests/test-template-chooser.ps1` - Template chooser dialog (9 tests)
- `tests/test-md-file-open.ps1` - .md file handler (7 tests)

### Documentation
- `README.md` - Repository overview
- `notes/README.md` - User documentation
- `notes/TODO.md` - Project roadmap
- `notes/DEV_NOTES.md` - This file
- `.claude/CLAUDE.md` - Claude Code project rules

---

**Last Updated**: 2026-02-14
**Development Session**: Phase 2 implementation complete
